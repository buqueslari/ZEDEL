...
// =====================================================
// PATCH FINAL - MOBILE DO ENTREGADOR + TELA DE ACOMPANHAMENTO
// Cole no FINAL do arquivo JS
// =====================================================

(function () {
    const FIXED_DRIVER_PHOTO = 'https://i.ibb.co/bjd0khXn/o-motoboy-alan-campos-oliveira-28-denuncia-que-abordagens-policiais-sao-motivadas-pela-cor-da-pele-1.jpg';
    ...
    function getCartTotalCents() {
        try {
            if (typeof cart !== 'undefined' && Array.isArray(cart)) {
                ...
                const discount = typeof couponApplied !== 'undefined' && couponApplied && typeof COUPON_DISCOUNT !== 'undefined'
                    ? COUPON_DISCOUNT
                    : 0;

                const shipping = typeof SHIPPING_FEE !== 'undefined' ? SHIPPING_FEE : 0;
    ...
    function getCurrentDriverData() {
        ...
        try {
            const saved = sessionStorage.getItem('orderDriverData');
            if (saved) driver = JSON.parse(saved);
        } catch (e) {}

        if (!driver && typeof getDailyDriverData === 'function') {
            driver = getDailyDriverData();
        }
        ...
        const photoFromScreen = el('driverPhoto')?.src;

        driver.photo = photoFromScreen || driver.photo || FIXED_DRIVER_PHOTO;

        return driver;
    }

    function saveCurrentDriverData() {
        try {
            const driver = getCurrentDriverData();

            const screenPhoto = el('driverPhoto')?.src;
            if (screenPhoto) driver.photo = screenPhoto;

            const screenName = el('driverName')?.innerText;
            if (screenName) driver.name = screenName;

            const screenRating = el('driverRating')?.innerText;
            if (screenRating) driver.rating = screenRating;

            const screenPlate = el('driverPlate')?.innerText;
            if (screenPlate) driver.plate = screenPlate;

            const screenVehicle = el('driverCar')?.innerText;
            if (screenVehicle) driver.vehicle = screenVehicle;

            const screenDistance = el('deliveryDistance')?.innerText;
            if (screenDistance) driver.distance = screenDistance.replace('km', '').trim();

            const screenTime = el('deliveryTime')?.innerText;
            if (screenTime) driver.time = screenTime.replace('~', '').replace('min', '').trim();

            sessionStorage.setItem('orderDriverData', JSON.stringify(driver));
        } catch (e) {}
    }

    function injectResponsiveCSS() {
        if (el('driverMobileAndTrackingStyle')) return;

        const style = document.createElement('style');
        style.id = 'driverMobileAndTrackingStyle';

        style.innerHTML = `
            /* ===== AJUSTE MOBILE DA TELA DO ENTREGADOR ===== */
            ...
            @media (max-width: 520px) {
                ...
                #driverFoundStep div[style*="height: 230px"] {
                    height: 310px !important;
                    border-radius: 18px !important;
                }

                #driverFoundStep .leaflet-popup {
                    display: none !important;
                }

                #driverFoundStep #driverPhoto,
                #driverFoundStep #driverInitialsFallback {
                    width: 74px !important;
                    height: 74px !important;
                    border-radius: 20px !important;
                }

                #driverFoundStep #driverName {
                ...
                #driverFoundStep #driverName + div {
                    gap: 6px !important;
                }

                #driverFoundStep #driverName + div span {
                    font-size: 11px !important;
                    padding: 5px 8px !important;
                }

                #driverFoundStep #driverName + div span:nth-child(2) {
                    display: none !important;
                }

                #driverFoundStep div[style*="grid-template-columns: repeat(3"] {
                    grid-template-columns: 1fr !important;
                }
            }

            /* ===== TELA DE ACOMPANHAMENTO ESTILO DELIVERY ===== */
            ...
            .tracking-header-icon {
                ...
                margin: 0 auto 12px;
                font-size: 34px;
                animation: trackingPulse 1.4s infinite;
            }
            ...
            .tracking-map {
                ...
                border-radius: 20px;
                background:
                    radial-gradient(circle at 22% 48%, #f59e0b 0 8px, transparent 9px),
                    radial-gradient(circle at 76% 52%, #22c55e 0 8px, transparent 9px),
                    linear-gradient(90deg, transparent 0 18%, rgba(15,23,42,.12) 18% 20%, transparent 20% 42%, rgba(15,23,42,.12) 42% 44%, transparent 44% 72%, rgba(15,23,42,.12) 72% 74%, transparent 74%),
                    linear-gradient(0deg, transparent 0 26%, rgba(15,23,42,.10) 26% 28%, transparent 28% 58%, rgba(15,23,42,.10) 58% 60%, transparent 60%),
                    #f1f5f9;
                position: relative;
                overflow: hidden;
                border: 1px solid #e2e8f0;
            ...
            .tracking-route {
                ...
                width: 52%;
                height: 4px;
                background: repeating-linear-gradient(90deg, #f59e0b 0 12px, transparent 12px 20px);
            ...
            .tracking-step.active .tracking-step-content h3 {
                color: #f59e0b;
            }
            ...
            .tracking-dots i:nth-child(2) {
                animation-delay: .15s;
            }

            .tracking-dots i:nth-child(3) {
                animation-delay: .3s;
            }
    ...
    function getTimeRange() {
        ...
        const end = new Date(now.getTime() + 28 * 60000);
    ...
    function displayDeliveryTrackingScreen() {
        injectResponsiveCSS();
        saveCurrentDriverData();

        if (window.__deliveryTrackingInterval) {
            clearInterval(window.__deliveryTrackingInterval);
        }
        ...
        const eta = getTimeRange();

        const stages = [
            {
                at: 0,
                title: 'Pedido confirmado',
                subtitle: 'Pagamento aprovado. Sua compra já entrou no sistema.',
                icon: '✓'
            },
            {
                at: 8,
                title: 'Entregador indo até a adega',
                subtitle: `${driver.name.split(' ')[0]} está a caminho do ponto de retirada.`,
                icon: '🏍️'
            },
            {
                at: 18,
                title: 'Pedido sendo separado',
                subtitle: 'A adega está separando seus produtos com cuidado.',
                icon: '🛒'
            },
            {
                at: 30,
                title: 'Retirada confirmada',
                subtitle: 'O entregador retirou o pedido na adega.',
                icon: '📦'
            },
            {
                at: 42,
                title: 'Saiu para entrega',
                subtitle: 'Agora o entregador está indo até o seu endereço.',
                icon: '🛵'
            },
            {
                at: 56,
                title: 'Chegando perto de você',
                subtitle: 'Fique atento ao telefone e ao local de entrega.',
                icon: '📍'
            },
            {
                at: 70,
                title: 'Pedido entregue',
                subtitle: 'Entrega finalizada. Obrigado pela compra!',
                icon: '✅'
            }
        ];
            ...
            <div class="tracking-page">
                <div class="tracking-card">
                    <div class="tracking-header">
                        <div class="tracking-header-icon">🛵</div>
                        <h1>Acompanhe seu pedido</h1>
                        <p>Olá, ${safeText(customerName)}. Seu pedido está em andamento.</p>
                    </div>

                    <div class="tracking-eta">
                        <div>
                            <small>Previsão de entrega</small>
                            <strong id="trackingEta">${safeText(eta)}</strong>
                        </div>

                        <div class="tracking-badge-live">
                            ● Atualizando em tempo real
                        </div>
                    </div>

                    <div class="tracking-driver">
                        <img src="${safeText(driver.photo || FIXED_DRIVER_PHOTO)}" alt="Foto do entregador">

                        <div>
                            <h3>${safeText(driver.name)}</h3>
                            <p>
                                ⭐ ${safeText(driver.rating)} · ${safeText(driver.vehicle)} · Placa ${safeText(driver.plate)}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="tracking-map">
                        ...
                        <div class="tracking-bike">🛵</div>
                        <div class="tracking-point store">Adega</div>
                        <div class="tracking-point home">Você</div>
                    </div>

                    <div class="tracking-timeline-wrap">
                        <div class="tracking-status-main">
                            <p>Status atual</p>
                            <h2 id="trackingStatusTitle">
                                Pedido confirmado
                                <span class="tracking-dots"><i></i><i></i><i></i></span>
                            </h2>
                        </div>

                        <div class="tracking-timeline">
                            <div class="tracking-line-bg"></div>
                            <div class="tracking-line-fill" id="trackingLineFill"></div>
                    ...
                    <div id="trackingDetails" class="tracking-details">
                        ...
                        ${safeText(address)}
                        <br><br>
                        <strong>Valor do pedido:</strong><br>
                        ${money(total)}
                        <br><br>
                        <strong>Entregador:</strong><br>
                        ${safeText(driver.name)} · ${safeText(driver.vehicle)} · ${safeText(driver.plate)}
                    </div>

                    <div class="tracking-actions">
                        <button type="button" class="tracking-btn secondary" id="toggleTrackingDetails">
                            Ver detalhes do pedido
                        </button>

                        <button type="button" class="tracking-btn" id="trackingRefreshButton">
                            Atualizar status
                        </button>
                    </div>
                </div>
        ...
        const stepsContainer = el('trackingSteps');
        const statusTitle = el('trackingStatusTitle');
        const lineFill = el('trackingLineFill');

        function getActiveIndex(seconds) {
            let active = 0;

            for (let i = 0; i < stages.length; i++) {
                if (seconds >= stages[i].at) {
                    active = i;
                }
            }
        ...
        function render(seconds) {
            const activeIndex = getActiveIndex(seconds);
            const activeStage = stages[activeIndex];
            const progress = Math.round((activeIndex / (stages.length - 1)) * 100);

            if (statusTitle) {
                const isFinal = activeIndex === stages.length - 1;

                statusTitle.innerHTML = `
                    ${safeText(activeStage.title)}
                    ${isFinal ? '' : '<span class="tracking-dots"><i></i><i></i><i></i></span>'}
                `;
            }
            ...
            if (stepsContainer) {
                stepsContainer.innerHTML = stages.map((stage, index) => {
                    ...
                    return `
                        <div class="tracking-step ${className}">
                            ...
                            <div class="tracking-step-content">
                                <h3>${safeText(stage.title)}</h3>
                                <p>${safeText(stage.subtitle)}</p>
                            </div>
                        </div>
                    `;
                }).join('');
            ...
            const bike = document.querySelector('.tracking-bike');
            if (bike) {
                if (activeIndex <= 1) {
                    bike.style.left = '22%';
        ...
        window.__deliveryTrackingInterval = setInterval(() => {
            seconds += 1;
            render(seconds);
        }, 1000);

        el('toggleTrackingDetails')?.addEventListener('click', function () {
            el('trackingDetails')?.classList.toggle('show');
        });

        el('trackingRefreshButton')?.addEventListener('click', function () {
            render(seconds + 1);

            this.innerText = 'Status atualizado ✓';

            setTimeout(() => {
                this.innerText = 'Atualizar status';
            }, 1200);
        });
    ...
    // Salva os dados do entregador quando a tela dele aparece
    const previousShowDriverFoundScreen = window.showDriverFoundScreen;

    if (typeof previousShowDriverFoundScreen === 'function') {
        window.showDriverFoundScreen = async function (...args) {
            const result = await previousShowDriverFoundScreen.apply(this, args);
    ...
    // Troca a tela final antiga pela nova tela de acompanhamento
    try {
        displaySuccessScreen = displayDeliveryTrackingScreen;
    } catch (e) {}

    window.displaySuccessScreen = displayDeliveryTrackingScreen;

    // Comando para testar no console:
    // showDeliveryTrackingDemo()
    window.showDeliveryTrackingDemo = displayDeliveryTrackingScreen;

    injectResponsiveCSS();
})();
// =====================================================
// PATCH URGENTE - BOTÃO DO ENTREGADOR IR PARA PIX/CARTÃO
// Cole no FINAL do arquivo JS
// =====================================================
...
(function () {
    ...
    function goDirectToPayment() {
        ...
        const documentInput = el('document');
        if (documentInput) {
            setTimeout(() => documentInput.focus(), 150);
        }
    ...
    const observer = new MutationObserver(function () {
        const button = el('continueToReviewButton');

        if (button) {
            button.innerText = 'Continuar para pagamento';
    ...
    window.goDirectToPayment = goDirectToPayment;
})();